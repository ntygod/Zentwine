...
本文0001–0005步骤描述ZT02-05原始组织能力，不是后续主线的完整安装清单。当前主线另有0006审计引用/索引和0007应急状态/回执；升级须按实际manifest核对已安装项，应用尚未安装的迁移并运行verify。0007授权须按应急指南更新，发生应急历史后禁止down；不能直接沿用上文0005的开发数据销毁说明跨过后续限制。

负责人现可使用[组织审计记录](organization-audit.md)及[成员应急访问控制](emergency-containment.md)。原“撤销组织会话”语义不变，不会自动升级为持续应急阻断；审计投影也不等于完整事件账本。当前交付与完整通道欠项见[ZT02-06执行记录](../tasks/ZT02-06-execution.md)。
