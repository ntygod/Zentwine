...
事件与业务事务同提交。after游标用于至少一次读取与去重，轮询没有硬实时SLA；退出登录或组织权限无效后不能继续读取。撤权判断不依赖客户端收到通知。审批者被停用/降权、身份和组织版本变化以及策略修改，通过合作治理入口原子撤销相关未完成审批。新数据库表写入失败时不能假装撤权成功。决定与领证在最后读取响应记录后复核期限和会话；显式撤销与通知读取同样复核会话。检查覆盖到最后一次事务内核验，不承诺数据库提交或网络返回瞬间的硬实时时限。
