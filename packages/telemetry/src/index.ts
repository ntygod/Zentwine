...
export function apiError(code: string, message: string, traceId: string, retryable = false): ApiErrorPayload {
  return { code, message, details: {}, trace_id: traceId, retryable };
}
const sensitive = /^(authorization|cookie|set-cookie|password|secret|token|api[_-]?key|credentials|database[_-]?url)$/i;
