...
const key = (s: OrganizationScope, serverKey: Buffer): Buffer =>
  createHmac("sha256", serverKey)
    .update("zentwine.organization-audit.cursor.v1\0" + s.session_digest)
    .digest();
...
export function encodeAuditCursor(
  s: OrganizationScope,
  q: OrganizationAuditQuery,
  p: Position,
  serverKey: Buffer,
): string {
  const iv = randomBytes(12),
    cipher = createCipheriv("aes-256-gcm", key(s, serverKey), iv);
...
export function decodeAuditCursor(
  ...
  try {
    ...
    if (raw.toString("base64url") !== q.cursor || raw.length < 29)
      throw new Error();
    ...
    if (
      !p ||
      Object.keys(p).sort().join(",") !== "before,expires,head,snapshot" ||
      !sequence(p.head) ||
      !sequence(p.before) ||
      BigInt(p.before) < 1n ||
      BigInt(p.before) > BigInt(p.head) ||
      typeof p.snapshot !== "string" ||
      !Number.isFinite(Date.parse(p.snapshot)) ||
      new Date(p.snapshot).toISOString() !== p.snapshot ||
      !Number.isSafeInteger(p.expires) ||
      p.expires !== Date.parse(p.snapshot) + CURSOR_TTL_MS ||
      !Number.isFinite(now) ||
      Date.parse(p.snapshot) > now ||
      p.expires <= now
    )
      throw new Error();
    return p;
...
export async function readOrganizationAudit(
  c: SqlConnection,
  s: OrganizationScope,
  q: OrganizationAuditQuery,
  serverKey: Buffer,
  ...
  const meta = (
    await c.query(
      `SELECT COALESCE(MAX(id),0)::text AS head,clock_timestamp() AS now FROM ${TABLE} WHERE org_id=$1`,
      [s.org_id],
    )
  ).rows[0];
  if (
    !meta ||
    !sequence(meta["head"]) ||
    !(meta["now"] instanceof Date) ||
    !Number.isFinite(meta["now"].getTime())
  )
  ...
  const p =
    q.cursor === undefined
      ? null
      : decodeAuditCursor(s, q, now.getTime(), serverKey);
  const head = p?.head ?? meta["head"],
    snapshot = p?.snapshot ?? now.toISOString();
  const rows = (
    await c.query(
      `SELECT id::text AS sequence,audit_ref,actor_id,kind,subject_id,created_at FROM ${TABLE}
    WHERE org_id=$1 AND id<=$2::bigint AND ($3::bigint IS NULL OR id<$3::bigint)
    AND ($4::text='all' OR kind=$4) ORDER BY id DESC LIMIT $5`,
      [s.org_id, head, p?.before ?? null, q.kind ?? "all", limit + 1],
    )
  ).rows;
  const entries: OrganizationAuditEntry[] = rows.slice(0, limit).map((r) => {
    ...
    if (
      !sequence(r["sequence"]) ||
      !isIdentityId(r["audit_ref"]) ||
      !isIdentityId(r["actor_id"]) ||
      !isIdentityId(r["subject_id"]) ||
      !ORGANIZATION_AUDIT_KINDS.includes(kind) ||
      !(r["created_at"] instanceof Date) ||
      !Number.isFinite(r["created_at"].getTime())
    )
      throw new OrganizationError("unavailable");
    return {
      reference: r["audit_ref"],
      occurred_at: r["created_at"].toISOString(),
      kind,
      actor_kind:
        kind === "identity.linked" || kind === "identity.provisioned"
          ? "identity_connection"
          : "human",
      actor_id: r["actor_id"],
      subject_kind: subjectKinds[kind],
      subject_id: r["subject_id"],
    };
  });
  const last = rows[limit - 1]?.["sequence"];
  const next =
    rows.length > limit && sequence(last)
      ? encodeAuditCursor(
          s,
          q,
          {
            ...
            before: last,
            snapshot,
            expires: p?.expires ?? now.getTime() + CURSOR_TTL_MS,
          },
          serverKey,
        )
      : null;
