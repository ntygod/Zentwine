...
import {
  encodeAuditCursor,
  decodeAuditCursor,
} from "../packages/db/dist/organization-audit.js";
...
const scope = () => ({
  session_digest: randomBytes(32).toString("hex"),
  org_id: randomUUID(),
  context_version: 2,
});
const position = (now) => ({
  head: "9007199254741009",
  before: "9007199254741008",
...
test("audit contract: domain and wire kinds agree and bounded queries validate", () => {
  ...
  for (const kind of ["all", ...ORGANIZATION_AUDIT_KINDS])
    validateAuditQuery({ kind, limit: 50 });
  validateAuditQuery({});
});
for (const [name, q] of Object.entries({
  ...
  array: [],
  string: "all",
  field: { org_id: randomUUID() },
  kind: { kind: "secret.dump" },
  zero: { limit: 0 },
  high: { limit: 51 },
  ...
  nan: { limit: NaN },
  cursor: { cursor: "../raw" },
  huge: { cursor: "a".repeat(1025) },
}))
  test("audit query rejects " + name, () =>
    assert.throws(() => validateAuditQuery(q), invalid),
  );
...
test("audit cursor: another session organization context filter or page size is rejected", () => {
  const s = scope(),
    now = Date.now(),
    cursor = encode(s, {}, position(now));
  for (const other of [
    { ...s, session_digest: scope().session_digest },
    { ...s, org_id: randomUUID() },
    { ...s, context_version: 3 },
  ])
    assert.throws(() => decode(other, { cursor }, now), invalid);
  for (const q of [
    { cursor, kind: "member.updated" },
    { cursor, limit: 1 },
  ])
...
test("audit cursor: ciphertext tampering noncanonical base64 expiry and invalid positions are rejected", () => {
  ...
  for (const bad of [
    { ...p, head: "9223372036854775808" },
    { ...p, before: "0" },
    { ...p, before: "9007199254741010" },
    { ...p, head: "001" },
    { ...p, expires: now + 900001 },
    { ...p, extra: true },
  ])
    assert.throws(
      () => decode(s, { cursor: encode(s, {}, bad) }, now),
      invalid,
    );
});
test("audit page: browser decoder accepts only the current organization and documented projection", () => {
  const p = sample();
  assert.deepEqual(parseOrganizationAuditPage(p, p.org_id), p);
  assert.throws(() => parseOrganizationAuditPage(p, randomUUID()), TypeError);
  for (const changed of [
    { ...p, complete_ledger: true },
    { ...p, total: 7 },
    { ...p, raw: "secret" },
    { ...p, entries: [...p.entries, ...p.entries] },
    { ...p, entries: [], next_cursor: "x".repeat(50) },
    { ...p, snapshot_at: "bad-date" },
    { ...p, next_cursor: "bad" },
  ])
    assert.throws(
      () => parseOrganizationAuditPage(changed, p.org_id),
      TypeError,
    );
});
test("audit page: unknown types raw payloads bad dates and false attribution are rejected", () => {
  ...
  for (const patch of [
    ...
    { actor_kind: "identity_connection" },
    { subject_kind: "human" },
    { actor_id: "email@example.test" },
    { occurred_at: "bad" },
    { payload: "token" },
  ])
    assert.throws(
...
test("audit cursor: cookie knowledge is insufficient without the original server instance key", () => {
  const s = scope(),
    ...
    cursor = encode(s, {}, position(now));
  assert.throws(
    () => decodeAuditCursor(s, { cursor }, now, randomBytes(32)),
    invalid,
  );
});
