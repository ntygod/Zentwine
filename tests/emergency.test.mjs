...
for (const [label, patch] of Object.entries({
  request: { request_id: "untrusted" },
  target: { confirm_human_id: "email@example.test" },
  negative: { expected_version: -1 },
  overflow: { expected_version: 2147483646 },
  fractional: { expected_version: 1.1 },
  member: { expected_member_version: 0 },
  string: { expected_member_version: "1" },
  action: { action: "restore_all" },
  wrong_reason: { reason: "incident_contained" },
  freeform: { reason: "secret narrative" },
  unknown: { role: "owner" },
}))
  test("emergency input rejects " + label, () =>
    assert.throws(
      () => validateEmergencyInput({ ...input(), ...patch }),
      (e) => e instanceof OrganizationError && e.code === "invalid_input",
    ),
  );
...
test("emergency state: decoder rejects forged scope extra data and impossible versions", () => {
  ...
  for (const patch of [
    ...
    { version: -1 },
    { member_version: 0 },
    { membership_status: "restored" },
  ])
    assert.throws(
      () => parseEmergencyState({ ...s, ...patch }, s.org_id, s.member_id),
      TypeError,
    );
});
test("emergency receipt: historical replay is distinct from the current released state", () => {
  const r = result();
  assert.deepEqual(
    parseEmergencyResult(r, r.current.org_id, r.current.member_id),
    r,
  );
  const replay = {
    ...r,
    replayed: true,
    current: { ...r.current, held: false, version: 2 },
  };
  assert.deepEqual(
    parseEmergencyResult(replay, r.current.org_id, r.current.member_id),
    replay,
  );
  assert.throws(
    () =>
