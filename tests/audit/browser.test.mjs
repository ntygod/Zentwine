...
test("audit browser PG: owner filters real records and pages a bounded snapshot on desktop and mobile", () =>
  fixture((f) =>
    browserTest(f, async ({ browser, base }) => {
      ...
      await expect(
        panel.getByRole("button", { name: "下一页审计记录" }),
      ).toBeDisabled();
      for (const value of await panel.locator("li small").allTextContents())
        assert.ok(!initialRefs.includes(value));
      ...
      await panel.getByRole("button", { name: "刷新审计记录" }).click();
      await expect(
        panel.getByText("当前筛选没有已记录事件。", { exact: true }),
      ).toBeVisible();
      assert.equal(
        await page.evaluate(
          () =>
...
test("audit browser PG: stale organization tab discards displayed history and viewer has no panel", () =>
  fixture((f) =>
    browserTest(f, async ({ browser, base }) => {
      await f.record();
      ...
      await expect(
        second.getByRole("region", { name: "组织审计记录" }),
      ).toBeVisible();
      ...
      await expect(
        page.getByRole("region", { name: "组织审计记录" }),
      ).toHaveCount(0);
      assert.ok(!(await page.textContent("body")).includes(reference));
    }),
  ));
test("audit browser PG: storage failure clears old results then revocation removes audit access", () =>
  fixture((f) =>
    browserTest(f, async ({ browser, base }) => {
      await f.record();
      const { page } = await login(f, browser, base, f.alice);
      ...
      await query(
        f.adminPool,
        `GRANT SELECT ON zentwine_organizations.events TO "${f.managerRole}"`,
      );
      await panel.getByRole("button", { name: "刷新审计记录" }).click();
      await expect(panel.getByRole("listitem")).toHaveCount(1);
