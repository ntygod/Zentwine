...
export function buildApp(options: AppOptions = {}) {
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    if (host && !/^(localhost|127\.0\.0\.1)(:\d+)?$/.test(host)) {
      return reply.code(403).send(apiError('forbidden', 'Local development access only', request.id));
    }
    if (request.headers['sec-fetch-site'] === 'cross-site') {
      return reply.code(403).send(apiError('forbidden', 'Cross-site access denied', request.id));
    }
  });
  app.setErrorHandler((error, request, reply) => {
    const inputError = error.validation !== undefined || (error.statusCode !== undefined && error.statusCode >= 400 && error.statusCode < 500);
  ...
  app.get('/api/v1/system/bootstrap', {
    schema: { querystring: { type: 'object', additionalProperties: false, properties: {} }, response: { 200: bootstrapSchema, 400: errorSchema } }
