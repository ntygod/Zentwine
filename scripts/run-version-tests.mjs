...
try {
  ...
  const result = spawnSync(
    process.execPath,
    ["--test", "--test-timeout=60000", "tests/versions/persistence.test.mjs"],
    { env, stdio: "inherit", timeout: 300000, killSignal: "SIGKILL" },
  );
  process.exitCode = result.status ?? 1;
} catch {
  console.error(
    "Version CAS tests require acknowledged disposable PostgreSQL and built tools; no integration was silently skipped.",
  );
  process.exitCode = 1;
}
