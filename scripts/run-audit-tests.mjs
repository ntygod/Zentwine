...
try {
  ...
  const files =
    mode === "--browser"
      ? ["tests/audit/browser.test.mjs"]
      : ["tests/audit/persistence.test.mjs", "tests/audit/http.test.mjs"];
  ...
  console.error(
    "Audit verification requires the acknowledged disposable database and built tools. No live identity provider or production audit ledger was certified.",
  );
  process.exitCode = 1;
}
